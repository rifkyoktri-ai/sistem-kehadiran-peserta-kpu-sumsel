    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onBatal}>
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="font-display font-bold text-xl text-[#0D1B3E]">Edit Data Peserta</h3>
        </div>

        <p className="font-mono text-xs text-[#003580] mb-6 bg-[#EEF2F7] px-3 py-2 rounded-lg">{peserta.id}</p>

        <div className="space-y-4">
          {fields.map((f) => (
            <div key={f}>
              <label className="block text-xs font-semibold text-[#0D1B3E] mb-1">{FIELD_LABEL[f]}</label>
              {f === 'catatan' ? (
                <textarea
                  value={form?.[f] ?? peserta[f] ?? ''}
                  onChange={(e) => handleChange(f, e.target.value)}
                  rows={3}
                  className="w-full border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#003580]"
                />
              ) : f === 'instansi' ? (
                <select
                  value={form?.[f] ?? peserta[f] ?? ''}
                  onChange={(e) => handleChange(f, e.target.value)}
                  className="w-full h-10 border border-[#E2E8F0] rounded-lg px-3 text-sm focus:outline-none focus:border-[#003580]"
                >
        ...
        <div className="flex gap-3 justify-end mt-8 pt-4 border-t border-[#E2E8F0]">
          <button
            onClick={onBatal}
            className="h-11 px-6 rounded-xl border border-[#E2E8F0] text-[#0D1B3E] font-semibold text-sm hover:bg-[#EEF2F7] transition"
          >
            Batal
          </button>
          <TombolPrimer
